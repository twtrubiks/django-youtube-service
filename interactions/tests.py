...
class CommentModelTests(TestCase):
    ...
    def _create_test_video(self, title, filename):
        ...
        return Video.objects.create(title=title, uploader=self.video_owner, video_file=dummy_video_file)

    def test_comment_creation(self):
        comment_content = "This is a great video!"
        comment = Comment.objects.create(video=self.video, user=self.user1, content=comment_content)
    ...
    def test_comment_reply_creation(self):
        parent_comment = Comment.objects.create(video=self.video, user=self.user1, content="Initial comment.")
        reply_content = "I agree with your comment!"
        reply = Comment.objects.create(
            video=self.video, user=self.user2, content=reply_content, parent_comment=parent_comment
        )
...
class LikeDislikeModelTests(TestCase):
    ...
    def test_like_creation(self):
        like = LikeDislike.objects.create(video=self.video, user=self.user1, type=LikeDislike.LIKE)
    ...
    def test_dislike_creation(self):
        dislike = LikeDislike.objects.create(video=self.video, user=self.user1, type=LikeDislike.DISLIKE)
...
class SubscriptionModelTests(TestCase):
    ...
    def test_subscription_creation(self):
        subscription = Subscription.objects.create(
            subscriber=self.user_subscriber, subscribed_to=self.user_channel_owner1
        )
    ...
    def test_user_cannot_subscribe_to_self(self):
        ...
        sub_to_self = Subscription.objects.create(subscriber=self.user_subscriber, subscribed_to=self.user_subscriber)
...
class AddCommentViewTests(TestCase):
    ...
    def test_add_comment_successful_non_ajax(self):
        ...
        expected_redirect_url = (
            reverse("videos:video_detail", kwargs={"video_id": self.video.id}) + f"#comment-{new_comment.id}"
        )
    ...
    def test_add_reply_to_comment_non_ajax(self):
        parent_comment = Comment.objects.create(
            video=self.video, user=self.video_author, content="Parent comment content"
        )
        reply_data = {"content": "This is a non-ajax reply.", "parent_comment_id": parent_comment.id}
        ...
        self.assertEqual(response.status_code, 302)
        expected_redirect_url = (
            reverse("videos:video_detail", kwargs={"video_id": self.video.id}) + f"#comment-{parent_comment.id}"
        )
        self.assertRedirects(response, expected_redirect_url)

    def test_add_reply_to_comment_ajax(self):
        parent_comment = Comment.objects.create(
            video=self.video, user=self.video_author, content="Parent for AJAX reply"
        )
        reply_data = {"content": "This is an AJAX reply.", "parent_comment_id": parent_comment.id}
        response = self.client.post(
            reverse("interactions:add_comment", args=[self.video.id]),
            data=reply_data,
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
    ...
    def test_add_comment_requires_login(self):
        self.client.logout()
        response = self.client.post(
            reverse("interactions:add_comment", args=[self.video.id]), data={"content": "Trying to comment"}
        )
        self.assertEqual(response.status_code, 302)
    ...
    def test_add_reply_to_non_existent_parent_comment_ajax(self):
        reply_data = {"content": "Reply to ghost parent.", "parent_comment_id": 8888}
        response = self.client.post(
            reverse("interactions:add_comment", args=[self.video.id]),
            data=reply_data,
            HTTP_X_REQUESTED_WITH="XMLHttpRequest",
...
class VoteVideoViewTests(TestCase):
    ...
    def test_dislike_video_ajax_first_time(self):
        ...
        self.assertTrue(
            LikeDislike.objects.filter(video=self.video, user=self.voter, type=LikeDislike.DISLIKE).exists()
        )
    ...
    def test_vote_video_non_ajax_redirects(self):
        response = self.client.post(
            reverse("interactions:vote_video", args=[self.video.id]), data={"vote_type": LikeDislike.LIKE}
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse("videos:video_detail", args=[self.video.id]))
...
class ToggleSubscriptionViewTests(TestCase):
    ...
    def test_subscribe_to_user_ajax_first_time(self):
        ...
        self.assertTrue(
            Subscription.objects.filter(subscriber=self.subscriber_user, subscribed_to=self.channel_owner).exists()
        )
    ...
    def test_subscribe_to_self_ajax(self):
        ...
        self.assertFalse(
            Subscription.objects.filter(subscriber=self.subscriber_user, subscribed_to=self.subscriber_user).exists()
        )

    def test_toggle_subscription_non_ajax_redirects(self):
        response = self.client.post(reverse("interactions:toggle_subscription", args=[self.channel_owner.id]))
        self.assertEqual(response.status_code, 302)
