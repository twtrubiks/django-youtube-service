...
def process_video(self, video_id):
    ...
    try:
        ...
        print(
            f"影片 {video.title} (ID: {video.id}) 開始轉檔... "
            f"原始路徑: {original_file_path}, 輸出路徑: {output_file_path_full}"
        )
        start_time_transcoding = time.time()
        try:
            stdout_bytes, stderr_bytes = (
                ffmpeg.input(original_file_path)
                .output(
                    output_file_path_full, vcodec="libx264", acodec="aac", strict="experimental", movflags="faststart"
                )
                .run(capture_stdout=True, capture_stderr=True, overwrite_output=True)
            )
            end_time_transcoding = time.time()
            print(
                f"影片 {video.title} (ID: {video_id}) 轉檔完成。"
                f"耗時: {end_time_transcoding - start_time_transcoding:.2f} 秒"
            )

            if stdout_bytes:
                print(
                    f"DEBUG: FFmpeg stdout (轉檔 {video_id}): {stdout_bytes.decode('utf8', errors='ignore')[:500]}..."
                )
            if stderr_bytes:
            ...
            print(
                f"DEBUG: Transcoding - ffmpeg.Error attribute type: {type(ffmpeg.Error if hasattr(ffmpeg, 'Error') else None)}"
            )
            if hasattr(ffmpeg, "Error"):
                print(f"DEBUG: Transcoding - inspect.isclass(ffmpeg.Error): {inspect.isclass(ffmpeg.Error)}")
            else:
                print("DEBUG: Transcoding - ffmpeg.Error attribute does not exist.")
        ...
        try:
            stdout_thumb_bytes, stderr_thumb_bytes = (
                ffmpeg.input(output_file_path_full)
                .output(thumbnail_output_file_path_full, vframes=1, format="image2", vcodec="mjpeg", ss="00:00:01.000")
                .run(capture_stdout=True, capture_stderr=True, overwrite_output=True)
            )
            end_time_thumbnail = time.time()
            print(
                f"INFO: 影片 {video.title} (ID: {video_id}) 縮圖產生完成。耗時: {end_time_thumbnail - start_time_thumbnail:.2f} 秒"
            )
            if stdout_thumb_bytes:
                print(
                    f"DEBUG: FFmpeg stdout (縮圖 {video_id}): {stdout_thumb_bytes.decode('utf8', errors='ignore')[:500]}..."
                )
            if stderr_thumb_bytes:
                print(
                    f"INFO: FFmpeg stderr (縮圖 {video_id}): {stderr_thumb_bytes.decode('utf8', errors='ignore')[:500]}..."
                )
            ...
            print(
                f"DEBUG_THUMBNAIL_ERROR: Caught exception during thumbnail generation. Type: {type(e).__name__}, Exception: {object.__repr__(e)}"
            )
            ...
            final_log_message = (
                f"ERROR: 影片 {video.title} (ID: {video.id}) 縮圖產生失敗: {error_message_for_print_and_return}"
            )
        ...
        print(
            f"DEBUG_GLOBAL_ERROR: Caught exception in global handler. Type: {type(e).__name__}, Exception: {object.__repr__(e)}"
        )
        ...
        if error_type_name == "TestSpecificFFmpegError" and hasattr(e, "stderr"):
            if hasattr(e.stderr, "_decode_return_value"):
                try:
                    ...
                    error_message_str = (
                        f"TestSpecificFFmpegError: [str(e.stderr._decode_return_value) failed: {inner_e}]"
                    )
            elif hasattr(e.stderr, "decode") and callable(e.stderr.decode):
                try:
        ...
        try:
            ...
            print(
                f"ERROR: 儲存影片 {video_id} (一般錯誤) 失敗狀態時發生錯誤: {type(save_err_generic).__name__} - {save_err_generic}"
            )
